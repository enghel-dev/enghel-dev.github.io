...
if (firebase?.auth && typeof db !== 'undefined') {
  ...
  console.warn('⚠️ Firebase Auth o Firestore (db) no está disponible aún. Verifica el orden de scripts.');
}
...
function loadUserPosts() {
  ...
  try {
    myPostsUnsub = db.collection('posts')
...
async function handleEditSubmit(e) {
  ...
  try {
    await db.collection('posts').doc(postId).update({
      title, category, content, image: image || null
    });
    closeEditModal();
...
async function migrateLocalToFirestore() {
  ...
  for (const key of candidates) {
    ...
    if (raw) {
      try { local = JSON.parse(raw); } catch (_) {}
  ...
  const normalize = (p) => {
    ...
    return {
      ...
      author,
      authorId,
      createdAt: createdAtLocal ? new Date(createdAtLocal) : firebase.firestore.FieldValue.serverTimestamp(),
